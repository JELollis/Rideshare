...
def delete_daily(db: Session, daily_id: int) -> bool:
    obj = get_daily(db, daily_id)
    if not obj:
        return False
    db.delete(obj)
...
def get_fuel_logs(db: Session, skip: int = 0, limit: int = 100):
    return (db.query(models.FuelLog)
...
def delete_fuel(db: Session, fuel_id: int) -> bool:
    obj = get_fuel(db, fuel_id)
    if not obj:
        return False
    db.delete(obj)
...
def create_user(db: Session, user: schemas.UserCreate):
    ...
    obj = models.User(
        ...
        driver_id=user.driver_id,  # can be None for admin-only accounts
...
def seed_states_if_empty(db: Session):
    ...
    STATES = [
        ...
        # ... (rest of states as in your file) ...
...
def replace_brackets(db: Session, state_id: int, year: int, filing_status: str, rows: list[dict]):
    ...
    for r in rows:
        b = models.StateTaxBracket(
            state_id=state_id, year=year, filing_status=filing_status,
            bracket_min=float(r["min"]),
            bracket_max=(None if r.get("max") in (None, "", "inf") else float(r["max"])),
            rate=float(r["rate"])
        )
        db.add(b)
    db.commit()
...
def load_default_brackets(db: Session, state_code: str, year: int, filing_status: str):
    ...
    state_def = next((s for s in blob.get("states", []) if s.get("code","").upper() == st.code), None)
    if not state_def:
        return False, f"No defaults for state {state_code} in {data_path.name}"
    rows = state_def.get("filing_status", {}).get(filing_status, [])
    if not rows:
        return False, f"No bracket set for {state_code} / {filing_status} in {year}"
    replace_brackets(db, st.id, year, filing_status, rows)
    return True, f"Loaded {len(rows)} bracket row(s) for {state_code} {year} {filing_status}"
